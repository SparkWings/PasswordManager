...
public class Main extends JFrame {
	...
	newMPwd.addActionListener(new ActionListener() {
		...
		frame.dispose();
		new ChangeMasterPasswordScreen(manager);
	...
	Object[][] data = new Object[manager.getPasswords().size()][];

	for (int i = 0; i < data.length; i++) {

	    int current = i;

	    Object[] o = new Object[] { manager.getPasswords().get(current).getName(), "View Password" };
	    data[current] = o;
	...
	table.addMouseListener(new MouseAdapter() {
		...
		if (table != null) {
			...
			IPassword i = manager.getPasswordForName(passwordName);

			new ChangePasswordScreen(manager, passwordName, i);
			frame.dispose();
